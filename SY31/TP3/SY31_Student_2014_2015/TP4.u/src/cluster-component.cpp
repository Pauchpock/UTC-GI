...
ClusterComponent::readScanFromInput(std::vector<vec2d>& points)
{
  ...
  // ** Note:
  //    Pour obtenir les valeurs des inputs, utilisez les fonctions suivantes
  //      angle(i) (en degrée):       angles->Float64(i)
  //      distances(i) (en mm):   distances->Integer32(i)
  for (int i = 0; i < angles->VectorSize(); ++i)
  { 
	  points[i].x = distances->Integer32(i)*cos(angles->Float64(i)*M_PI/180);
      points[i].y = distances->Integer32(i)*sin(angles->Float64(i)*M_PI/180);
  }
}
