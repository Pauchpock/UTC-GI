...
OdometryComponent::Core() 
{
  ...
  double dt = (MAPS::CurrentTime()- mPreviousTimestamp)/1e6;
  ...
  vLeft = (vLeft*TWOPI*WIFIBOT_RADIUS_IN_METER) /NB_TICKS_PER_WHEEL_TURN;
  vRight = (vRight*TWOPI*WIFIBOT_RADIUS_IN_METER) /NB_TICKS_PER_WHEEL_TURN;
 
  // Update the odometry.
  mWifibot.updateOdometry(dt, vLeft, vRight);
