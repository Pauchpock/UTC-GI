/*  Atelier Photo - Travaux Pratiques UV MI01
 Copyright (C) 2005, 2014 S. Bonnet, Université de Technologie de Compiègne
...
/* ProcessedBitmap : bitmap utilisé pour le traitement */
typedef struct {
  UINT biWidth;       /* Largeur */
  UINT biHeight;    /* Hauteur */
  BYTE *pBits;        /* Tableau de pixels  */
 ...
 */
ProcessedBitmap *CreateProcessedBitmap(UINT lWidth, UINT lHeight, WORD wBitCount) {
  BITMAPINFO *pbmi;
  ...
  if (wBitCount == 8) {
    ...
    pbmih->biClrImportant = 256;
      ...
      pColorTable = (RGBQUAD*)((LPSTR)pbmi + (WORD)(pbmi->bmiHeader.biSize));
  ...
  if (hBitmap != NULL) {
    bitmap = (ProcessedBitmap *) malloc(sizeof(ProcessedBitmap));
                             ...
                             LPARAM lParam)
{
  ...
  enum { PROCESS_IMAGE_NONE, PROCESS_IMAGE_C, PROCESS_IMAGE_MMX, PROCESS_IMAGE_ASM } processToLaunch = PROCESS_IMAGE_NONE;
      ...
      switch(LOWORD(wParam)) {
        case IDM_FILEOPEN:
          if ((lpszFile = GetBitmapFileName(hwnd)) != NULL) {
            ...
            if (mybm == NULL) {
              MessageBox(hwnd,
                TEXT("Impossible d'ouvrir le fichier image. Vérifiez qu'il s'agit bien d'un fichier BMP 24 ou 32 bits.\n"),
        ...
        case IDM_SHOWDEST:
          ...
          break;
            ...
            case IDM_LAUNCH_C:
              processToLaunch = PROCESS_IMAGE_C;
        case IDM_LAUNCH_MMX:
                if (processToLaunch == PROCESS_IMAGE_NONE) {
                  processToLaunch = PROCESS_IMAGE_MMX;
          }
        case IDM_LAUNCH_ASM:
                if (processToLaunch == PROCESS_IMAGE_NONE) {
                  processToLaunch = PROCESS_IMAGE_ASM;
          }

              /* Supprimer les bitmaps existants s'ils existent */
          if (Bitmaps[1] != NULL) {
            DeleteProcessedBitmap(Bitmaps[1]);
          }

          if (Bitmaps[2] != NULL) {
            DeleteProcessedBitmap(Bitmaps[2]);
          }

          if (Bitmaps[3] != NULL) {
            DeleteProcessedBitmap(Bitmaps[3]);
          }

              /* Créer les bitmaps avec le bon format */
              if (processToLaunch == PROCESS_IMAGE_MMX) {
