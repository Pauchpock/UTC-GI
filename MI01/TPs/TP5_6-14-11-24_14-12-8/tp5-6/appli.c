...
MenuDescription MenuDescriptions[] = {
  ...
  {IDM_LAUNCH_ASM, TEXT("Lancer le traitement en assembleur")},
 ...
 *
 */
UINT process_image_C(UINT biWidth, UINT biHeight, BYTE *img_src,
                     BYTE *img_temp1, BYTE *img_temp2, BYTE *img_dest)
{
  unsigned int c, n;
  unsigned int x, y;
...
ProcessedBitmap *CreateProcessedBitmap(UINT biWidth, UINT biHeight)
{
  ...
  if (hBitmap != NULL) {
    bitmap = malloc(sizeof(ProcessedBitmap));
